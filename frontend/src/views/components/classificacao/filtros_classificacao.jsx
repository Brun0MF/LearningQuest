...
const FiltrosClass = ({categorias = [], categoriaId = "", topicosCategoria = [], topicoId = "", topicoDisabled = true, onChangeCategoria, onChangeTopico, onApply}) => {
    ...
    return(
        <div className="flex sm:flex-row flex-col gap-3 justify-between">
            <select onChange={onChangeCategoria} value={categoriaId} id="" className="sm:w-6/12 w-full py-2 rounded-xl border border-verdeSuave-600 px-2 focus:border-verdeSuave focus:ring-2 focus:ring-verdeSuave outline-none transition-all">
                <option value="" selected disabled>Categoria</option>
                {categorias.map((cat) => (
                    <option key={cat.id_categoria} value={cat.id_categoria}>{cat.titulo_categoria}</option>
                ))}
            </select>
            <select disabled={topicoDisabled} onChange={onChangeTopico} value={topicoId} className="sm:w-3/6 w-full py-2 rounded-xl border border-verdeSuave-600 px-2 focus:border-verdeSuave focus:ring-2 focus:ring-verdeSuave outline-none transition-all">
